...
-- Fix pagination and ordering in get_catalog_specialists function
...
BEGIN
    ...
    -- Get featured specialists (up to 3 per page, randomized)
    featured_specialists AS (
        SELECT fd.*, tc.total_rows, 1 as display_order
    ...
    -- Get regular specialists for remaining slots
    regular_specialists AS (
        SELECT fd.*, tc.total_rows, 2 as display_order
        FROM filtered_data fd
        CROSS JOIN total_count_calc tc
        WHERE fd.is_featured_specialist = false
        ORDER BY fd.specialist_name
        LIMIT (p_page_size - (SELECT COUNT(*) FROM featured_specialists))
        OFFSET offset_val
    ),
    -- Combine results
    combined_results AS (
    ...
    ORDER BY cr.display_order, cr.is_featured_specialist DESC, cr.specialist_name;
END;
$$;
